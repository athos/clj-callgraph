...
(defn dump-data
  ...
  ([src-files opts]
   (with-output opts #(prn (ana/analyze src-files)))))
...
(defn render-graph
  ...
  ([dump-file opts]
   (with-output opts #(render/render % (read-dump-file dump-file)))))
...
(defn generate-graph
  ...
  ([src-files opts]
   (with-output opts #(render/render % (ana/analyze src-files)))))
